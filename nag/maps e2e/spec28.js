describe("login functionality", function() {
    browser.driver.manage().window().maximize();

    //browser.ignoreSynchronization = true;
     it('user info', function() {
       //browser.get('https://dya4247t866iy.cloudfront.net');
        browser.get('https://chatapp.aadhya-analytics.com:4201');
         browser.waitForAngularEnabled(false)
       element(by.xpath('//*[@id="Username"]')).sendKeys("nagaraju");
       element(by.id("Password")).sendKeys("nag");
     
     
       element(by.buttonText("Sign In")).click();
       browser.driver.sleep(40000);
       });
 
    it("ddsdsdsd",function()
    {
      element(by.xpath('//*[@id="navbar"]/ul/li[3]/a')).click()
      browser.driver.sleep(40000);
      element(by.xpath(' //*[@id="leftscroll"]/ul[4]/li[103]/div')).click()
      browser.driver.sleep(40000);
//   browser.waitForAngularEnabled(false)
//          element(by.xpath('/html/body/aadhya-root/app-chat-page/div/div/div/div/div/aadhya-map-page/div/div[1]/usersearch/div/div[2]/button/span')).click();
//      browser.driver.sleep(40000);
//      element(by.xpath('/html/body/aadhya-root/app-chat-page/div/div/div/div/div/aadhya-map-page/div/div[1]/usersearch/div/div[2]/div/form/div/ul/li[2]span')).click();
//      browser.driver.sleep(40000);
        
    });

    it("ddsdsdsd",function()
    {
    //   element(by.xpath('//*[@id="navbar"]/ul/li[3]/a')).click()
    //   browser.driver.sleep(40000);
    //   element(by.xpath(' //*[@id="leftscroll"]/ul[4]/li[103]/div')).click()
    //   browser.driver.sleep(40000);
  browser.waitForAngularEnabled(false)
         element(by.xpath('/html/body/aadhya-root/app-chat-page/div/div/div/div/div/aadhya-map-page/div/div[1]/usersearch/div/div[2]/button/i')).click();

     browser.driver.sleep(40000);
    //  element(by.xpath('//*[@id="User"]')).sendKeys('madhu');
    //  element(by.xpath('/html/body/aadhya-root/app-chat-page/div/div/div/div/div/aadhya-map-page/div/div[1]/usersearch/div/div[2]/div/form/div/ul/li[2]span')).click();
    });
    it("ddsdsdsd",function()
    {
   
     element(by.xpath('//*[@id="User"]')).sendKeys('ram');
     browser.driver.sleep(40000);
     element(by.xpath('/html/body/aadhya-root/app-chat-page/div/div/div/div/div/aadhya-map-page/div/div[1]/usersearch/div/div[2]/div/form/div/ul/li[2]')).click();
     browser.driver.sleep(40000);
//      let list = $$('.list-group list-group');
// expect(list.count()).toBe(0);
    //  browser.actions().click(protractor.Button.LEFT).perform();
    //  browser.driver.sleep(40000);
    //  element(by.xpath('/html/body/aadhya-root/app-chat-page/div/div/div/div/div/aadhya-map-page/div/div[1]/usersearch/div/div[2]/div/form/div/ul/li[6]')).click();
     //element(by.xpath('')).$('[value="raheemmujavar"]').click();
//      browser.driver.sleep(40000);
//      var select = element.one(by.model('orderProp'));
// select.$('[value="age"]').click();
//      browser.driver.sleep(40000);
    //  element(by.xpath('/html/body/aadhya-root/app-chat-page/div/div/div/div/div/aadhya-map-page/div/div[1]/usersearch/div/div[2]/div/form/div/ul/li[2]span')).click();
    //  browser.driver.sleep(40000);
    //  browser.actions().click(protractor.Button.LEFT).perform();
    //  browser.driver.sleep(40000);
    });
    // it("ddsdsdsd",function()
    // {
   
    // //  element(by.xpath('//*[@id="User"]')).sendKeys('madhu');
    // // //  browser.actions().click(protractor.Button.LEFT).perform();
    //  browser.driver.sleep(40000);

    //  element(by.xpath('/html/body/aadhya-root/app-chat-page/div/div/div/div/div/aadhya-map-page/div/div[1]/usersearch/div/div[2]/div/form/div/ul/li[1]')).click();
    //  browser.driver.sleep(40000);

    //  browser.actions().click(protractor.Button.LEFT).perform();
    //  browser.driver.sleep(40000);
    //  browser.actions().click(protractor.Button.LEFT).perform();
    //  browser.driver.sleep(40000);
    // });

});